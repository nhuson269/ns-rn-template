...
class UserService {
  ...
  async login(username: string, password: string): Promise<LoginDemoResult> {
    try {
      const response: ApiResponse<any> = await this.client.instance.post(routes.demo.herokuapp.user.login, {
        email: username,
        password: password,
      });
      if (!response.ok) {
  ...
  async register(fullname: string, email: string, password: string): Promise<RegisterDemoResult> {
    try {
      const response: ApiResponse<any> = await this.client.instance.post(routes.demo.herokuapp.user.register, {
        name: fullname,
        email: email,
        password: password,
        age: 18,
  ...
  async logout(): Promise<LogoutDemoResult> {
    try {
      const response: ApiResponse<any> = await this.client.instance.post(routes.demo.herokuapp.user.logout);
      if (!response.ok) {
        const problem = getGeneralApiProblem(response);
        if (problem) {
          return problem;
  ...
  async userMe(): Promise<GetDemoUserResult> {
    try {
      const response: ApiResponse<any> = await this.client.instance.get(routes.demo.herokuapp.user.me);
      if (!response.ok) {
        const problem = getGeneralApiProblem(response);
        if (problem) {
          return problem;
