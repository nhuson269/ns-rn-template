...
const makeAnimatedValue = (switchOn: boolean) => new Animated.Value(switchOn ? 1 : 0);

export function Switch(props: SwitchProps) {
  const value = props.value ?? false;
  const [timer] = React.useState<Animated.Value>(makeAnimatedValue(value ?? false));
  ...
  const [previousValue, setPreviousValue] = React.useState<boolean>(value ?? false);
  React.useEffect(() => {
  ...
  const trackStyle = [
    ...
    value ? props.trackOnStyle : props.trackOffStyle,
  ];
  ...
  const thumbStyle = [
    ...
    value ? props.thumbOnStyle : props.thumbOffStyle,
  ];
